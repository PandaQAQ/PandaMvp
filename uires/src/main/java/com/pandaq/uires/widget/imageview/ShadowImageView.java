...
public class ShadowImageView extends AppCompatImageView {
    ...
    public ShadowImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.ShadowImageView, defStyle, 0);
        if (ta != null) {
            mRadius = ta.getDimension(R.styleable.ShadowImageView_image_radius, 0);
            mShadowRadius = ta.getDimension(R.styleable.ShadowImageView_image_shadow_radius, 0);
            mIsCircle = ta.getBoolean(R.styleable.ShadowImageView_image_circle, false);
            mIsShadow = ta.getBoolean(R.styleable.ShadowImageView_image_shadow, false);
            mShadowColor = ta.getColor(R.styleable.ShadowImageView_shadow_color, Color.parseColor("#e4e4e4"));
            ta.recycle();
        } else {
            mRadius = 0;
            mShadowRadius = 0;
    ...
    private Bitmap createCircleImage(Bitmap bitmap) {
        ...
        targetCanvas.drawCircle(imageWidth / 2f, imageWidth / 2f, Math.min(imageWidth, imageHeight) / 2f,
                mPaint);
        ...
        if (mIsShadow) {
            ...
            canvas.drawCircle(width / 2f, height / 2f, Math.min(imageWidth, imageHeight) / 2f,
                    mPaint);
            mPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC_OVER));
            mPaint.setShadowLayer(0, 0, 0, 0xffffff);
            canvas.drawBitmap(targetBitmap, mShadowRadius, mShadowRadius, mPaint);
