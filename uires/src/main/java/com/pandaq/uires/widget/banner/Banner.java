package com.pandaq.uires.widget.banner;
...
import com.pandaq.uires.widget.banner.transformer.MZScaleInTransformer;
import com.pandaq.uires.widget.banner.transformer.ScaleInTransformer;
import com.pandaq.uires.widget.banner.util.BannerLifecycleObserver;
import com.pandaq.uires.widget.banner.util.BannerLifecycleObserverAdapter;
import com.pandaq.uires.widget.banner.util.BannerUtils;
import com.pandaq.uires.widget.banner.util.ScrollSpeedManger;
...
public class Banner<T, A extends BannerAdapter<T, ? extends RecyclerView.ViewHolder>> extends FrameLayout implements BannerLifecycleObserver {
    ...
    static class AutoLoopTask implements Runnable {
        ...
        public void run() {
            ...
            if (banner != null && banner.mIsAutoLoop) {
                ...
                if (banner.getAdapter() != null && banner.getAdapter().getRealCount() > 0) {
                    Object item = banner.getAdapter().getRealData(next);
                    if (item instanceof SelfLoopTimeItem) {
                        SelfLoopTimeItem data = (SelfLoopTimeItem) item;
                        long selfPlayTime = data.getPlayTime();
                        if (selfPlayTime < 3000) {
                            selfPlayTime = 3000;
                        }
                        if (selfPlayTime > BannerConfig.LOOP_TIME_MAX) {
                            selfPlayTime = BannerConfig.LOOP_TIME_MAX;
                        }
                        banner.postDelayed(banner.mLoopTask, selfPlayTime);
                    } else {
    ...
    public Banner start() {
        if (mIsAutoLoop) {
            stop();
            // 如果是 item 自带独立展示时间则以 item 时间为准
            int startPosition = isInfiniteLoop() ? mStartPosition : 0;
            if (getAdapter() != null && getAdapter().getRealCount() > 0) {
                Object item = getAdapter().getRealData(startPosition);
                if (item instanceof SelfLoopTimeItem) {
                    SelfLoopTimeItem data = (SelfLoopTimeItem) item;
                    long selfPlayTime = data.getPlayTime();
                    if (selfPlayTime < 3000) {
                    ...
                    postDelayed(mLoopTask, selfPlayTime);
                    return this;
                }
            }
     ...
     * 设置banner的适配器
     */
    public Banner setAdapter(A adapter) {
        if (adapter == null) {
     ...
     * @return
     */
    public Banner setAdapter(A adapter, boolean isInfiniteLoop) {
