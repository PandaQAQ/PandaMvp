...
fun BigDecimal.showPrice2Num(): SpannableString {
    ...
    if (value.contains(".")) {
        spannableString.setSpan(RelativeSizeSpan(0.6f), value.indexOf("."), value.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
    }
...
fun TextView.setGradientStyle(@ColorInt colorStart: Int, @ColorInt colorEnd: Int) {
    val gradient = LinearGradient(0f, 0f, paint.textSize * text.length, 0f, colorStart, colorEnd, Shader.TileMode.CLAMP)
...
fun String.formatPriceStyle(): SpannableString {
    ...
    if (this.contains(".")) {
        spannableString.setSpan(RelativeSizeSpan(0.8f), this.indexOf("."), this.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
    }
    return spannableString
}
